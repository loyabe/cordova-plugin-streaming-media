...
public abstract class GSYVideoPlayer extends GSYBaseVideoPlayer implements View.OnClickListener, View.OnTouchListener, SeekBar.OnSeekBarChangeListener, TextureView.SurfaceTextureListener {
    ...
    protected boolean mIsComplete; //是否开始不让拖动。

    public void setSeekPosstion(int position){
        mCurrentPosition = position;
        mlastPosition = position;
        mSeekToInAdvance = position;
    }
            ...
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    if (mChangePosition) {
                        ...
                        if(!mIsComplete && mSeekTimePosition > mlastPosition){
                            mSeekTimePosition = (int)mlastPosition;
                        }
    ...
    public void onStopTrackingTouch(SeekBar seekBar) {
        ...
        if (GSYVideoManager.instance().getMediaPlayer() != null && GSYVideoManager.instance().getMediaPlayer().isPlaying()) {
            ...
            if (!mIsComplete && time > mlastPosition){
                time = (int)mlastPosition;
            }
    ...
    public void onAutoCompletion() {
        ...
        if (mGSYVideoPlayerListener != null)
            mGSYVideoPlayerListener.onAutoCompletion();
    }
    ...
    public void onBufferingUpdate(int percent) {
        if (mlastPosition < getCurrentPositionWhenPlaying())
            mlastPosition = getCurrentPositionWhenPlaying();
        ...
        if (GSYVideoManager.instance().lastListener() != null){
            GSYVideoManager.instance().lastListener().onBufferingUpdate(percent);
        }
    }
    ...
    public void onError(int what, int extra) {
        ...
        if (mGSYVideoPlayerListener != null){
            mGSYVideoPlayerListener.onError(what, extra);
        }
    }
    ...
    private GSYVideoPlayerListener mGSYVideoPlayerListener;
    public void setGSYVideoPlayerListener(GSYVideoPlayerListener listener){
        mGSYVideoPlayerListener = listener;
    }
    protected void setTextAndProgress(int secProgress) {
        ...
        if (mGSYVideoPlayerListener != null)
            mGSYVideoPlayerListener.onProgressAndTime(progress, secProgress, position, duration);
